...
const NativePayments = {
  supportedGateways: IS_ANDROID ? ['stripe', 'braintree'] // On Android, Payment Gateways are supported out of the gate.
  : ReactNativePayments ? ReactNativePayments.supportedGateways : [],
  canMakePayments(methodData) {
    return new Promise((resolve, reject) => {
      if (IS_ANDROID) {
        ReactNativePayments.canMakePayments(methodData, err => reject(err), canMakePayments => resolve(true));
        return;
      }
  ...
  show(methodData, details, options = {}) {
    return new Promise((resolve, reject) => {
      if (IS_ANDROID) {
        ReactNativePayments.show(methodData, details, options, err => reject(err), (...args) => {
          console.log(args);
          resolve(true);
        });
        return;
  ...
  getFullWalletAndroid(googleTransactionId, paymentMethodData, details) {
    return new Promise((resolve, reject) => {
      if (!IS_ANDROID) {
        reject(new Error('This method is only available on Android.'));
        return;
      }
      ReactNativePayments.getFullWalletAndroid(googleTransactionId, paymentMethodData, details, err => reject(err), serializedPaymentToken => resolve({
