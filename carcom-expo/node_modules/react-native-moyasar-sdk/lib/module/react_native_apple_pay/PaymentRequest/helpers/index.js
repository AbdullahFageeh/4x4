...
import { DOMException, ConstructorError } from "../errors/index.js";
...
export function convertObjectAmountToString(objectWithAmount) {
...
export function convertDetailAmountsToString(details) {
  const nextDetails = Object.keys(details).reduce((acc, key) => {
    ...
    if (Array.isArray(details[key]) && (key === 'displayItems' || key === 'shippingOptions')) {
      return Object.assign({}, acc, {
...
export function getPlatformMethodData(methodData, platformOS) {
...
export function validateTotal(total, errorType = Error) {
...
export function validatePaymentMethods(methodData) {
  ...
  methodData.forEach(paymentMethod => {
    ...
    const serializedData = paymentMethod.data ? JSON.stringify(paymentMethod.data) : null;
    serializedMethodData.push([paymentMethod.supportedMethods, serializedData]);
  });
  return serializedMethodData;
}
export function validateDisplayItems(displayItems, errorType = Error) {
  ...
  if (displayItems) {
    displayItems.forEach(item => {
      ...
      if (!isValidDecimalMonetaryValue(amountValue)) {
        throw new errorType(`'${amountValue}' is not a valid amount format for display items`);
      }
    });
  }
}
export function validateShippingOptions(details, errorType = Error) {
  ...
  if (details.shippingOptions) {
    ...
    details.shippingOptions.forEach(shippingOption => {
      ...
      if (seenIDs.includes(shippingOption.id)) {
        ...
        console.warn(`[ReactNativePayments] Duplicate shipping option identifier '${shippingOption.id}' is treated as an invalid address indicator.`);
        return undefined;
      }
...
export function hasGatewayConfig(platformMethodData = {}) {
  ...
  if (typeof platformMethodData.paymentMethodTokenizationParameters.parameters !== 'object') {
    return false;
  }
  if (!platformMethodData.paymentMethodTokenizationParameters.parameters.gateway) {
    return false;
  }
  if (typeof platformMethodData.paymentMethodTokenizationParameters.parameters.gateway !== 'string') {
    return false;
  }
  return true;
}
export function getGatewayName(platformMethodData) {
  return platformMethodData.paymentMethodTokenizationParameters.parameters.gateway;
}
export function validateGateway(selectedGateway = '', supportedGateways = []) {
  if (!supportedGateways.includes(selectedGateway)) {
    throw new ConstructorError(`"${selectedGateway}" is not a supported gateway. Visit https://goo.gl/fsxSFi for more info.`);
  }
}
