...
import { CreditCardConfig } from "./credit_card_config.js";
/**
...
export class PaymentConfig {
  ...
  constructor({
    ...
    assert(publishableApiKey.length > 0, 'Please fill `publishableApiKey` argument with your key.');
    assert(Number.isInteger(amount), 'Amount must be an integer.');
    assert(amount > 0, 'Amount must be a positive integer.');
    assert(merchantCountryCode.length > 0, 'Please fill `merchantCountryCode` argument (ISO 3166-1 alpha-2 country code).');
    assert(currency.length > 0, 'Please fill `currency` argument (ISO 4217 3-letter currency code).');
    assert(description.length > 0, 'Please add a description.');
    assert(supportedNetworks.length > 0, 'At least 1 network must be supported.');
    ...
    this.supportedNetworks = supportedNetworks.map(network => network.toLowerCase());
...
//# sourceMappingURL=payment_config.js.map