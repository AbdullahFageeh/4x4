...
import { SamsungPayButton } from "./index.js";
...
import { SamsungPayRequestSource } from "../../models/api/sources/samsung_pay/samsung_pay_request_source.js";
import { createPayment } from "../../services/payment_service.js";
import { GeneralError, isMoyasarError, NetworkError } from "../../models/errors/moyasar_errors.js";
import { toMajor } from "../../helpers/currency_util.js";
import { assert } from "../../helpers/assert.js";
import { SamsungPayButtonType } from "../../models/component_models/samsung_pay_button_type.js";
import { jsx as _jsx } from "react/jsx-runtime";
export async function onSamsungPayResponse(token, orderNumber, paymentConfig, onPaymentResult) {
  ...
  try {
    ...
    if (isMoyasarError(error)) {
      ...
      onPaymentResult(new NetworkError('Moyasar SDK: An error occured error while processing a Samsung Pay payment'));
    }
  }
}
...
'CreateSamsungPayButtonFragment', [viewId]);
export function SamsungPay({
  paymentConfig,
  onPaymentResult,
  style
}) {
  assert(!!paymentConfig.samsungPay, 'Samsung Pay is not configured to use Samsung Pay, you have to configure the `samsungPay` property in the `PaymentConfig` object');
  ...
  if (!isSamsungDevice() || !paymentConfig.samsungPay) {
    debugLog('Moyasar SDK: Samsung Pay is not supported on this device or the `samsungPay` property is not set, showing empty view');
  ...
  return /*#__PURE__*/_jsx(View, {
    ...
    children: /*#__PURE__*/_jsx(SamsungPayButton, {
      style: {
      ...
      merchantInfo: {
        ...
        // Currently, the merchantId will be part of the publishable API key
        ...
        buttonType: style?.buttonType ?? SamsungPayButtonType.payWithSamsungPay
      },
      onPaymentResult: result => {
        if (!result.nativeEvent.result) {
          errorLog('Moyasar SDK: Samsung Pay result is empty or null');
          onPaymentResult(new GeneralError('Moyasar SDK: Samsung Pay token is null'));
          return;
        }
        const payload = JSON.parse(result.nativeEvent.result ?? '');
        onSamsungPayResponse(payload['3DS'].data, result.nativeEvent.orderNumber ?? '', paymentConfig, onPaymentResult);
      },
      ref: ref
    })
  });
}
//# sourceMappingURL=samsung_pay.js.map