...
export function CreateCommunityScreen({ navigation }: Props) {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) return setError('اسم المجتمع مطلوب');
    if (!carModel.trim()) return setError('موديل السيارة مطلوب');
    if (!description.trim()) return setError('وصف المجتمع مطلوب');
    ...
    setLoading(false);
    if (community) navigation.replace('CommunityDetail', { communityId: community.id });
  };
        ...
        <View style={styles.inputGroup}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
            {carModels.map((m) => (
              <TouchableOpacity
                key={m}
                onPress={() => setCarModel(m)}
                style={[styles.chip, { backgroundColor: carModel === m ? theme.colors.primary : theme.colors.surface, borderColor: theme.colors.border }]}>
                <Text style={[styles.chipText, { color: carModel === m ? theme.colors.onPrimary : theme.colors.text }]}>
                  {m}
                </Text>
              </TouchableOpacity>
            ))}
