...
export function PlaceSearchScreen({ navigation }: Props) {
  ...
  const handleSearch = async () => {
    ...
    setHasSearched(true);
    try {
      const suggestions = await placesService.searchPlaces(query, selectedTypes.length > 0 ? selectedTypes : undefined);
      const fullPlaces: Place[] = [];
      for (const s of suggestions) {
        const details = await placesService.getPlaceDetails(s.place_id);
        ...
        {Object.entries(PLACE_CATEGORIES).map(([key, value]) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={[styles.filterChip, { backgroundColor: isSelected ? theme.colors.primary : theme.colors.surface, borderColor: theme.colors.border }]}>
              <Text style={[styles.filterText, { color: isSelected ? theme.colors.onPrimary : theme.colors.text }]}>
    ...
    <View style={[styles.placeCard, { backgroundColor: theme.colors.card, borderColor: theme.colors.border }]}>
      <View style={styles.placeInfo}>
        ...
        <View style={styles.placeMeta}>
          ...
          {item.rating && (
            <Text style={[styles.metaText, { color: theme.colors.accent }]}>⭐ {item.rating} ({item.review_count})</Text>
          )}
        </View>
    ...
    if (!hasSearched) {
      return (
        <View style={styles.centerContainer}>
          ...
          <Text style={[styles.emptyText, { color: theme.colors.textMuted }]}>محطات وقود، مطاعم، استراحات، مواقع تخييم</Text>
        </View>
      );
    }
    return (
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  titleBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 60, paddingBottom: 16 },
  backButton: { fontSize: 24 },
  title: { fontSize: 20, fontWeight: '600' },
  header: { paddingHorizontal: 20, paddingBottom: 16 },
  searchContainer: { flexDirection: 'row', alignItems: 'center', gap: 12, height: 52, borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, marginBottom: 16 },
  ...
  placeCard: { flexDirection: 'row', borderRadius: 16, borderWidth: 1, padding: 16, marginBottom: 12 },
  placeInfo: { flex: 1 },
  placeHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
  placeName: { fontSize: 16, fontWeight: '600', flex: 1 },
  verifiedBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  verifiedText: { fontSize: 10, fontWeight: '600' },
  placeDesc: { fontSize: 14, marginBottom: 8 },
  placeMeta: { flexDirection: 'row', gap: 16, marginBottom: 8 },
  metaText: { fontSize: 12 },
  placeTypes: { flexDirection: 'row', gap: 6, flexWrap: 'wrap' },
  typeTag: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, borderWidth: 1 },
  typeText: { fontSize: 10 },
  addButton: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', alignSelf: 'center' },
