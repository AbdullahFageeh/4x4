...
export function TripDetailScreen({ navigation, route }: Props) {
  ...
  const { currentTrip, participants, isLoading, error, fetchTrip, fetchParticipants, joinTrip } = useTripStore();
  ...
  const isFull = currentTrip.current_participants >= currentTrip.participant_limit;
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.coverContainer}>
        ...
        <View style={[styles.badge, { backgroundColor: theme.colors.primary }]}>
          <Text style={[styles.badgeText, { color: theme.colors.onPrimary }]}>
            {category?.label_ar || currentTrip.category}
          </Text>
        </View>
        {isFull && (
      ...
      <View style={styles.content}>
            ...
            <View style={styles.infoContent}>
              ...
              <Text style={[styles.infoValue, { color: theme.colors.text }]}>
                ...
                {spotsLeft > 0 && spotsLeft <= 3 && (
                  <Text style={{ color: theme.colors.warning }}> ({spotsLeft} أماكن متبقية)</Text>
                )}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.infoContent}>
              ...
              <Text style={[styles.infoValue, { color: theme.colors.accent }]}>
                {currentTrip.estimated_cost} {currentTrip.cost_currency}
              </Text>
            </View>
          </View>
        </View>
        ...
        {currentTrip.itinerary && (
          <View style={styles.section}>
            ...
            <View style={[styles.itineraryCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              <Text style={[styles.itineraryText, { color: theme.colors.textMuted }]}>{currentTrip.itinerary}</Text>
            </View>
          </View>
        )}
        ...
        {currentTrip.preparation_checklist && currentTrip.preparation_checklist.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={[styles.checklistCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
        ...
        {currentTrip.route_stops && currentTrip.route_stops.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={[styles.stopsCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            ...
            {participants.length === 0 ? (
              ...
              <>
                <Text style={[styles.participantStats, { color: theme.colors.textMuted }]}>
                  ✅ مشارك: {goingCount}  •  ⏳ قائمة الانتظار: {waitlistCount}
                </Text>
                <View style={styles.avatarRow}>
                  ...
                  {participants.length > 8 && (
                    <View style={[styles.avatarPlaceholder, { backgroundColor: theme.colors.disabled }]}>
                      <Text style={{ color: theme.colors.textMuted, fontSize: 12 }}>+{participants.length - 8}</Text>
                    </View>
                  )}
                </View>
              </>
      ...
      <View style={[styles.footer, { backgroundColor: theme.colors.background, borderTopColor: theme.colors.border }]}>
        <TouchableOpacity
          onPress={() => navigation.navigate('TripPayments', { tripId: currentTrip.id })}
          style={[styles.paymentButton, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <Text style={[styles.paymentText, { color: theme.colors.primary }]}>{t('payment.payNow')} ({currentTrip.estimated_cost} SAR)</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={handleJoin}
...
const styles = StyleSheet.create({
  ...
  itineraryText: { fontSize: 14, lineHeight: 22 },
  checklistCard: {
    borderRadius: 12,
    borderWidth: 1,
    padding: 16,
