...
export function CreateTripScreen({ navigation }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    const trip = await createTrip({
      community_id: 'comm-001', // In production, user selects community
      title: title.trim(),
      ...
      meeting_point: {
        name: meetingPoint.trim() || destination.trim(),
        lat: 0,
        lng: 0,
      },
      route_stops: [],
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.form}>
        <View style={styles.inputGroup}>
          <Text style={[styles.label, { color: theme.colors.text }]}>{t('trip.create')}</Text>
          <TextInput
            style={[
              styles.input,
            ...
            {TRIP_CATEGORIES.map((cat) => (
              <TouchableOpacity
                ...
                <Text
                  style={[
                    styles.chipText,
                    { color: category === cat.key ? theme.colors.onPrimary : theme.colors.text },
                  ]}>
