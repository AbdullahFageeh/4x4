...
export const DEMO_PLACES: Place[] = [
  ...
  {
    place_id: 'place-002',
    name: 'استراحة وادي ديسي',
    description: 'استراحة ومطعم في مدخل وادي ديسي، منظر خلاب',
    types: ['rest_area', 'restaurant'],
    location: { lat: 29.0, lng: 36.0 },
    rating: 4.5,
    review_count: 89,
    is_verified: true,
    saudi_region: 'تبوك',
  ...
  },
  {
    ...
    types: ['campsite', 'attraction'],
    location: { lat: 21.3, lng: 40.3 },
    rating: 4.6,
    review_count: 445,
    is_verified: true,
    saudi_region: 'الطائف',
  },
  {
    ...
    types: ['attraction', 'restaurant'],
    location: { lat: 21.7, lng: 39.1 },
    rating: 4.7,
    review_count: 1250,
    is_verified: true,
    saudi_region: 'جدة',
...
// Demo directions from Riyadh to common Saudi destinations
export const DEMO_DIRECTIONS: Record<string, RouteDirection> = {
  'riyadh-tabuk': {
    ...
    steps: [
      ...
      { instruction: 'Merge onto Highway 65 (Northern Ring Road)', distance: '15 km', duration: '15 mins' },
      { instruction: 'Continue on Highway 65 towards Al-Qassim', distance: '400 km', duration: '4 hours' },
      { instruction: 'Pass through Buraidah (fuel stop recommended)', distance: '', duration: '' },
  ...
  'riyadh-jeddah': {
    ...
    steps: [
      ...
      { instruction: 'Merge onto Highway 40 (Makkah Highway)', distance: '50 km', duration: '30 mins' },
      { instruction: 'Continue on Highway 40 to Jeddah', distance: '900 km', duration: '8 hours 30 mins' },
      { instruction: 'Pass through Taif (fuel stop recommended)', distance: '', duration: '' },
      { instruction: 'Arrive at Jeddah', distance: '', duration: '' },
    ],
  },
  'riyadh-taif': {
    distance: '780 km',
    ...
    steps: [
      ...
      { instruction: 'Continue towards Taif', distance: '725 km', duration: '7 hours 20 mins' },
      { instruction: 'Arrive at Taif/Al-Shafa turnoff', distance: '', duration: '' },
    ],
  },
};

// Simulated Places API service
class PlacesService {
  ...
  async getNearbyPlaces(lat: number, lng: number, radius: number = 50, types?: PlaceType[]): Promise<Place[]> {
    await this.delay();
    // Simple distance-based filter (mock)
    let results = DEMO_PLACES.filter((p) => {
      const dist = Math.sqrt(
        Math.pow(p.location.lat - lat, 2) + Math.pow(p.location.lng - lng, 2)
      );
      return dist < (radius / 111); // rough km to degrees
    });
